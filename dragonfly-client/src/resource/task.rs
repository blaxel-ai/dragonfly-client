...
/// The delay before the p2p throughput is first compared against
/// `download.minP2pThroughput`, so that the connection setup and the first
/// pieces do not count as a slow transfer.
const MIN_P2P_THROUGHPUT_GRACE_PERIOD: Duration = Duration::from_secs(3);

/// How often the p2p throughput is compared against `download.minP2pThroughput`.
const MIN_P2P_THROUGHPUT_CHECK_INTERVAL: Duration = Duration::from_secs(1);

/// The outcome of downloading pieces from the parents, and whether the caller
/// should give up on p2p and fall back to the source.
struct ParentDownloadResult {
    finished_pieces: Vec<metadata::Piece>,
    should_back_to_source: bool,
}

/// Returns the elapsed time in tenths of a second, the unit the throughput
/// comparison works in to stay in integer arithmetic.
fn elapsed_deciseconds(elapsed: Duration) -> u64 {
    elapsed.as_secs() * 10 + (elapsed.subsec_millis() / 100) as u64
}

/// Returns the average throughput in bytes per second over the elapsed time.
...
/// Returns whether the p2p throughput has stayed below the configured minimum,
/// once the grace period has passed. A minimum of zero disables the check.
